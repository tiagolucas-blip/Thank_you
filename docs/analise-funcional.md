...
- **Auditoria técnica (pós-Fase 8)**: revisão completa a toda a camada de serviços, API e configuração, a pedido do utilizador. Corrigido:
    ...
    - **Seletor de idioma inexistente**: i18n PT/EN/FR existia desde a Fase 1 mas só era alcançável via `?sap-language=` na URL — nenhum utilizador o descobria. Adicionado `languageSelect` em `Home.view.xml`, persistido em `webapp/service/LanguagePreference.ts` (localStorage). A troca é sempre feita com reload completo (nunca em runtime — `Localization.setLanguage()` não invalida o `ResourceBundle` já cacheado em `Component.ts`), aplicado via `document.write()` em `index.html` para embutir `data-sap-ui-language` na própria tag de bootstrap antes desta arrancar — confirmado que modificar o atributo depois de criada (`nextElementSibling`) não funciona, porque a tag ainda não existe no DOM nesse momento da análise síncrona do HTML.

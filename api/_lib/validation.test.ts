...
void test("requisito 5 — assertNotSelfRecognition rejeita autor igual ao destinatário", () => {
    assert.throws(() => assertNotSelfRecognition("00000001", "00000001"), ValidationError);
});

void test("requisito 5 — assertNotSelfRecognition aceita autor diferente do destinatário", () => {
    assert.doesNotThrow(() => assertNotSelfRecognition("00000001", "00000002"));
});

void test("requisito 5 — assertValidSubmission rejeita auto-elogio mesmo com o resto do payload válido", () => {
    assert.throws(() => assertValidSubmission(validSubmission({ recipientId: "00000001" })), ValidationError);
});

void test("assertValidSubmission rejeita classificação fora do intervalo 1-5", () => {
    assert.throws(
...
void test("assertValidSubmission aceita um payload válido", () => {
